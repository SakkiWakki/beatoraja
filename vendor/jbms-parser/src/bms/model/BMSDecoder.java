...
public class BMSDecoder extends ChartDecoder {
	...
	private static final Charset CHARSET_UTF8 = java.nio.charset.StandardCharsets.UTF_8;

	/**
	 * Detect the charset of a BMS file from its raw bytes.
	 * Checks for UTF-8 BOM, then validates as UTF-8, then distinguishes
	 * EUC-KR from Shift-JIS by byte pattern analysis.
	 */
	private static Charset detectCharset(byte[] data) {
		...
		// Try UTF-8 validation — if all multi-byte sequences are valid UTF-8, use it
		...
		while (i < data.length) {
			...
			if (i + seqLen > data.length) { validUtf8 = false; break; }
			for (int j = 1; j < seqLen; j++) {
				if ((data[i + j] & 0xC0) != 0x80) { validUtf8 = false; break; }
			}
			if (!validUtf8) break;
			i += seqLen;
		}
		...
		// Distinguish EUC-KR from Shift-JIS by scoring byte patterns.
		// EUC-KR: lead byte 0xA1-0xFE, trail byte 0xA1-0xFE
		// Shift-JIS: lead byte 0x81-0x9F or 0xE0-0xFC, trail byte 0x40-0x7E or 0x80-0xFC
		int eucKrScore = 0;
		int sjisScore = 0;
		for (i = 0; i < data.length - 1; i++) {
			...
			boolean isEucKr = b1 >= 0xA1 && b1 <= 0xFE && b2 >= 0xA1 && b2 <= 0xFE;
			boolean isSjis = ((b1 >= 0x81 && b1 <= 0x9F) || (b1 >= 0xE0 && b1 <= 0xFC))
					&& ((b2 >= 0x40 && b2 <= 0x7E) || (b2 >= 0x80 && b2 <= 0xFC));

			if (isEucKr && !isSjis) eucKrScore++;
			else if (isSjis && !isEucKr) sjisScore++;
			// When both match, check for Shift-JIS half-width katakana (0xA1-0xDF single byte)
			// which is very common in Japanese BMS files
			else if (b1 >= 0xA1 && b1 <= 0xDF && b2 < 0x80) sjisScore++;

			if (isEucKr || isSjis) i++; // skip trail byte
		}
	...
	private BMSModel decode(Path path, byte[] data, boolean ispms, int[] selectedRandom) {
		...
		try (BufferedReader br = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(data), detectedCharset))) {
			...
			final IntervalCoverage[] completedCoverage = new IntervalCoverage[model.getMode().key];
			...
			for (Section section : sections) {
				section.makeTimeLines(wm, bm, timelines, lnlist, completedCoverage, laneHistory, lnendstatus);
			}
			// Logger.getGlobal().info(
